...
public class PushChannelHandler implements NotificationChannelHandler {
    ...
    public ChannelDeliveryResult deliver(NotificationEntity notification) {
        if (!StringUtils.hasText(notificationProperties.getPushGatewayUrl())) {
            return ChannelDeliveryResult.failure("Push gateway URL is not configured");
        }
        if (!StringUtils.hasText(notification.getRecipientAddress())) {
            return ChannelDeliveryResult.failure("Push device token is required in recipientAddress");
        }
        try {
