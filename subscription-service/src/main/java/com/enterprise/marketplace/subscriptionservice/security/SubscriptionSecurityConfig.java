package com.enterprise.marketplace.subscriptionservice.security;
...
public class SubscriptionSecurityConfig {
    ...
    @Bean
    SecurityFilterChain subscriptionSecurityFilterChain(
        ...
        if (securityEnabled) {
            http.authorizeHttpRequests(auth -> auth.requestMatchers(
                                    ...
                                    "/api/v1/bootstrap/**")
                            .permitAll()
                            .requestMatchers(HttpMethod.GET, "/api/v1/plans/**")
                            .permitAll()
                            .requestMatchers(HttpMethod.POST, "/api/v1/plans/**")
                            .hasRole(MarketplaceRoles.ADMIN)
                            .requestMatchers(HttpMethod.PUT, "/api/v1/plans/**")
                            .hasRole(MarketplaceRoles.ADMIN)
                            .requestMatchers(HttpMethod.DELETE, "/api/v1/plans/**")
                            .hasRole(MarketplaceRoles.ADMIN)
